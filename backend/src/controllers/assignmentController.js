...
export async function postAssignmentHandler(req, res, next) {
  try {
    ...
    if (role !== 'admin' && classData.faculty_id !== userId) {
      return res.status(403).json({ error: 'Forbidden: You are not authorized to post an assignment.' });
    }
    ...
    `;
    const values = [class_id, module_id, title.trim(), description.trim(), due_date, max_points, userId];
...
export async function deleteAssignmentIdHandler(req, res, next) {
  try {
    const { id: assignmentId } = req.params;
    const { id: userId, role } = req.user;
